...
describe('DataTable', function() {
  ...
  describe('#fetchHistory', function() {
    ...
    describe('when the value references are restricted', function() {
      ...
      beforeAll(function(done) {
        this.dataTable.createCell('history cell', { value: 0 })
          ...
          .then(cell => new Promise(resolve => {
            // Adds 1ms before updating here and below to protect against race conditions
          ...
          .then(cell => new Promise(resolve => {
            setTimeout(() => resolve(cell.update({ value: 4 })), 1);
          }))
          .then(done, done.fail);
      });

      beforeAll(function(done) {
        this.dataTable.fetchHistory({ startTime: this.startTime, endTime: this.endTime })
          .then(({ original, transformed }) => {
            this.original = original;
            this.transformed = transformed;
          })
